...
### Editing (`PUT /api/itinerary/:itineraryId`)
`updateItinerary` (protected, owner-only, same 403 pattern as `shareItinerary`) accepts a full replacement `daysPlan` — `[{ day, attractions: [placeId, ...] }]` — for drag-and-drop reordering/moving attractions between days. It re-derives `morning` (first 3) / `evening` (rest) and `attractionCount` per day using the same convention as `generateItinerary`, rather than trusting client-supplied splits. Validation is strict: the submitted days must be numbered `1..itinerary.days` in order, every attraction id must be a valid ObjectId, and — critically — the **multiset of attraction ids across all days must exactly match** what the itinerary already had (checked via a `Set` built from the existing `daysPlan`) before the write is allowed. This means the endpoint can only rearrange attractions that already belong to the itinerary; it can't be used to inject arbitrary `Place` ids or silently drop one during a drag operation — any mismatch (missing id, duplicate id, foreign id) is rejected with 400 before anything is saved.

On the frontend, `ItineraryPage.jsx`'s "Edit Order" toggle uses `@dnd-kit/core` + `@dnd-kit/sortable` for cross-day drag-and-drop, not Framer Motion's `Reorder` (already used elsewhere in the app) — `Reorder` only supports reordering within a single list, not moving items *between* day columns, which is the whole point of this feature. `attractionsByDay` (keyed by `"Day N"`, built inside `buildItineraryTimeline` from each day's full `attractions` array, separately from the richer `timeline` structure used for the normal read view) is the drag source of truth; dragging moves attractions between/within these plain arrays via `handleDragEnd`'s `findDayContainingAttraction` + `arrayMove`. Entering edit mode swaps the whole itinerary body for a simplified grid of day columns (`DroppableDayColumn`/`SortableAttraction`) — no traffic connectors, action cards, or section headers. A parallel `initialAttractionsByDay` snapshot (set at the same point as `attractionsByDay`) drives an `isOrderDirty` check that gates the "Save Changes" button, and clicking "Done Editing" without saving discards the drag by resetting `attractionsByDay` back to that snapshot rather than persisting anything. "Save Changes" converts `attractionsByDay` into the `daysPlan` shape the `PUT` endpoint expects and calls it; on success it re-runs `buildItineraryTimeline` with the server's response (which also re-fetches traffic for the now-different legs and rewrites the `localStorage` cache blob, same as a fresh generation), and that re-run's `setEditMode(false)`/snapshot update is what naturally exits edit mode and clears the dirty state — there's no separate "exit edit mode" call after a save.
