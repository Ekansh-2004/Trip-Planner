...
On the frontend, `ItineraryPage.jsx`'s "Edit Order" toggle (currently **UI + local state only — not yet wired to `PUT`**) uses `@dnd-kit/core` + `@dnd-kit/sortable` for cross-day drag-and-drop, not Framer Motion's `Reorder` (already used elsewhere in the app) — `Reorder` only supports reordering within a single list, not moving items *between* day columns, which is the whole point of this feature. `attractionsByDay` (keyed by `"Day N"`, built inside `buildItineraryTimeline` from each day's full `attractions` array, separately from the richer `timeline` structure used for the normal read view) is the drag source of truth; dragging moves attractions between/within these plain arrays via `handleDragEnd`'s `findDayContainingAttraction` + `arrayMove`. Entering edit mode swaps the whole itinerary body for a simplified grid of day columns (`DroppableDayColumn`/`SortableAttraction`) — no traffic connectors, action cards, or section headers — and exiting resets automatically whenever a fresh `buildItineraryTimeline` runs (new generation or history load), since edits aren't persisted yet.
